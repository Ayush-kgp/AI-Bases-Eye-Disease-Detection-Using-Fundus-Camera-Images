"""Export trained MobileNet-V2 PyTorch checkpoint to ONNX format."""
...
def export_mobilenet(
    ...
    print(f"[OK] MobileNet exported successfully -> {output_path}")


if __name__ == "__main__":
    base_dir = Path(__file__).resolve().parent.parent.parent
    default_ckpt = base_dir.parent / "training-artifacts" / "mobilenet_unfrozen_best.pth"
    default_out = base_dir / "ml" / "models" / "mobilenet.onnx"

    ckpt_path = sys.argv[1] if len(sys.argv) > 1 else str(default_ckpt)
    out_path = sys.argv[2] if len(sys.argv) > 2 else str(default_out)

    export_mobilenet(ckpt_path, out_path)
